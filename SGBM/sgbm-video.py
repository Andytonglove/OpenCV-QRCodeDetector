...
while ret:
    ...
    # 鼠标回调事件
    cv2.imshow("depth", dis_color)  # 还没有depth窗口不能生成鼠标回调事件，挪上来
    cv2.setMouseCallback("depth", onmouse_pick_points, threeD)  # 报错

    # 完成计时，计算帧率
    fps = (fps + (1.0 / (time.time() - t1))) / 2
    frame = cv2.putText(
        frame, "fps= %.2f" % (fps), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
    )
