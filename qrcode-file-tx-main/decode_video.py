...
while True:
    ...
    if lazy_frame == True:
        frame = frame[
            lazy_frame_pts[0][0][1] : lazy_frame_pts[2][0][1],
            lazy_frame_pts[0][0][0] : lazy_frame_pts[2][0][0],
        ]
    ...
    for obj in decoded_objects:
        ...
        if int(byte_data[0]) == (QRCODE_NUMBER - 1):
            ...
            if total_frame == -1:
                total_frame = bytes_with_length_prefix_to_int(
                    byte_data[2 + frame_index_len :]
                )
            ...
            if frame_index > current_frame:
                ...
                if frame_index != current_frame + 1:
                    print(
                        f"Error: Lost frame {current_frame + 1}. Current frame: {frame_index}, last frame: {current_frame}"
                    )
                    set_retry()
                    print(
                        f"Retry from frame {current_frame} with image arg: Binary_Thresh={binary_thresh}, Alpha={reset_alpha}"
                    )
                frame_ok = True
                for i in range(QRCODE_NUMBER - 1):
                    if i not in current_frame_data:
                        print(
                            f"Error: Missing QR code {i} for frame {current_frame}, will retry"
                        )
                        frame_ok = False
                if frame_ok == True:
                    ...
                    print(
                        f"New frame index {frame_index} detected. Set current_frame={frame_index}."
                    )
                    Found_All_QR_in_Frame = False
                    if (
                        lazy_frame == True
                    ):  # If current frame is lazy frame, go back to the last frame and redetect the current frame for the data QR codes.
                        cap.set(
            ...
            if chunk_frame == current_frame:
                if chunk_index not in current_frame_data:
                    ...
                    if len(chunk_data) != chunk_data_len:
                        print(
                            f"Error: Wrong chunk data length: {len(chunk_data)} != {chunk_data_len} in QR code {chunk_index} for frame {current_frame}"
                        )
                        exit()
                    current_frame_data[chunk_index] = chunk_data
                    print(
                        f"Adding QR code {chunk_index} for frame:{current_frame}/{total_frame}, QR:{len(current_frame_data)}/{QRCODE_NUMBER - 1}, chunk size: {chunk_data_len}"
                    )
            else:
                print(
                    f"Warning: Wrong frame index: {chunk_frame} != {current_frame} in QR code {chunk_index}, ignored!"
                )
