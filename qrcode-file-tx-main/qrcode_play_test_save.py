...
class QRCodeWidget(QWidget):
    ...
    def update_qrcodes(self):
        ...
        for i, (chunk, label) in enumerate(zip(chunks, self.labels)):
            img = None
            if curent_frame == -1:  # End of File, generate end QR code
...
def main():
    ...
    if Cahce_available == False:
        ...
        frames_number = max(frames_count)
        if not all_elements_equal(frames_count):
            print(
                "Warning! Number of frames generated for QR codes are not equal! Some QR codes may be lost!"
            )
        ...
        print("Load QR codes from cache.")
