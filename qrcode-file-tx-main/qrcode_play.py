...
def format_chunk(content, current_split_index):
    ...
    for i in range(num_full_chunks):
        print(
            f"\rGenerating chunk {i}/{num_full_chunks} for QR Code:{current_split_index + 1}/{QRCODE_NUMBER - 1}      ",
            end="",
            flush=True,
        )
        ...
        formatted_chunk = (
            bytes([current_split_index])
            + int_to_bytes_with_length_prefix(i)
            + bytes([CHUNK_SIZE])
    ...
    if remaining_bytes > 0:
        print(
            f"\rGenerating chunk {num_full_chunks}/{num_full_chunks} for QR Code:{current_split_index + 1}/{QRCODE_NUMBER - 1}      ",
            end="",
            flush=True,
        )
        ...
        formatted_chunk = (
            bytes([current_split_index])
            + int_to_bytes_with_length_prefix(len(formatted_splits))
...
class QRCodeWidget(QWidget):
    def __init__(self):
        ...
        for i, label in enumerate(self.labels):
            layout.addWidget(
                label, i // QR_CODE_NUMBER_PER_ROW, i % QR_CODE_NUMBER_PER_ROW
            )
    ...
    def update_qrcodes(self):
        ...
        for i, (chunk, label) in enumerate(zip(chunks, self.labels)):
            ...
            if (
                curent_frame == -1
            ):  # End of File, generate end QR code (All QR Code data = 0xFFFFAA003CFF00FF)
                ...
                if i == (QRCODE_NUMBER - 1):  # last QR code for sync
                    ...
                    img = generate_qr(
                        qr_index_bytes
                        + int_to_bytes_with_length_prefix(curent_frame)
                        + int_to_bytes_with_length_prefix(frames_number)
                    ).convert("RGB")
                else:  # data QR codes
...
def main():
    ...
    if os.path.exists("qr_cache.pickle"):
        ...
        try:
            if (
                loaded_data["QRCODE_NUMBER"] != QRCODE_NUMBER
                or loaded_data["CHUNK_SIZE"] != CHUNK_SIZE
                or loaded_data["QRCODE_SIZE"] != QRCODE_SIZE
            ):
    ...
    if Cahce_available == False:
        ...
        chunks.append(
            0
        )  # The last one for the last QR code for sync, it will be generated during playing
        ...
        if all_elements_equal(frames_count):
            print(
                f"All QR codes have the same number of frames:{frames_count[0]}. Good!"
            )
